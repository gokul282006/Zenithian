...
export default function SocialPresentation() {
  ...
  // Selected platforms checkboxes (Default: X, Instagram, LinkedIn, Facebook, PowerPoint)
  const [selectedPlatforms, setSelectedPlatforms] = useState(['x', 'instagram', 'linkedin', 'facebook', 'powerpoint']);
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Choose which channels to generate grounded content for simultaneously</p>
            </div>
            ...
            <div className="flex items-center gap-1.5 flex-wrap text-xs">
              ...
              <button
                type="button"
                onClick={() => setSelectedPlatforms(['x', 'instagram', 'linkedin', 'facebook', 'powerpoint'])}
              ...
              <button
                type="button"
                onClick={() => setSelectedPlatforms(['x', 'instagram', 'linkedin', 'facebook'])}
                className="px-2.5 py-1 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 font-semibold transition-colors"
              >
                Social Only
              </button>
              <button
                type="button"
                onClick={() => setSelectedPlatforms(['powerpoint'])}
                className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 font-semibold transition-colors"
              >
                PPTX Only
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3.5">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2 font-bold text-sm">
                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${selectedPlatforms.includes('x') ? 'bg-white text-black' : 'bg-slate-200 text-slate-800'}`}>
                    <XIcon className="w-3.5 h-3.5" />
                  </div>
                  <span>𝕏 (Twitter)</span>
                </div>
                {selectedPlatforms.includes('x') && <Check className="w-4 h-4 text-sky-400" />}
              </div>
              <p className={`text-xs ${selectedPlatforms.includes('x') ? 'text-slate-300' : 'text-slate-500'}`}>
                Punchy 280-char posts & 3-tweet threads (1/n) with verified hashtags.
              </p>
            </div>
            ...
            <div
              ...
              className={`p-4 rounded-2xl border cursor-pointer transition-all hover-lift ${
                selectedPlatforms.includes('instagram')
                  ? 'bg-gradient-to-br from-pink-50 to-rose-50 border-pink-400 shadow-sm'
            ...
            >
              ...
              <p className="text-xs text-slate-600">Caption, curated tags & 3-slide visual carousel structure.</p>
            </div>
            ...
            >
              ...
              <p className="text-xs text-slate-600">Executive perspectives, bullet points & business insights.</p>
            </div>
            ...
            >
              ...
              <p className="text-xs text-slate-600">Canva-inspired 16:9 widescreen presentation slide deck.</p>
            </div>
          </div>
        </div>
